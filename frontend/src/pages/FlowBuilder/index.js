...
const FlowBuilder = () => {
  ...
  const handleDuplicateFlow = async flowId => {
    try {
      ...
      toast.success("Flujobot duplicado con éxito");
    } catch (err) {
      toastError(err);
    }
  };
  ...
  return (
    <MainContainer className={classes.mainContainer}>
      ...
      >
        {deletingContact
          ? `¿Estás seguro de que quieres eliminar este Flujobot? Se perderán todas las integraciones relacionadas.`
          : `${i18n.t("contacts.confirmationModal.importMessage")}`}
      </ConfirmationModal>
      <ConfirmationModal
        title={
          deletingContact
            ? `¿Quieres duplicar Flujobot? ${deletingContact.name}?`
            : `${i18n.t("contacts.confirmationModal.importTitlte")}`
        }
      ...
      >
        {deletingContact
          ? `¿Estás seguro de que quieres duplicar este Flujobot?`
          : `${i18n.t("contacts.confirmationModal.importMessage")}`}
      </ConfirmationModal>
      <MainHeader>
        <Title>Flujobots conversacionales</Title>
          ...
          <>
            ...
            >
              <MenuItem onClick={() => {
                handleClose()
                hadleEditContact()
                }}>Editar nombre</MenuItem>
              <MenuItem onClick={() => {
                handleClose()
                exportLink()
                }}>Editar Flujobot</MenuItem>
              <MenuItem onClick={() => {
                handleClose()
                setConfirmDuplicateOpen(true);
                }}>Duplicado</MenuItem>
