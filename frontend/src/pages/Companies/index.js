...
//     }, []);
    ...
    const renderWhatsapp = (row) => {
        return row.useWhatsapp === false ? "No" : "Si";
    };

    const renderFacebook = (row) => {
        return row.useFacebook === false ? "No" : "Si";
    };

    const renderInstagram = (row) => {
        return row.useInstagram === false ? "No" : "Si";
    };

    const renderCampaigns = (row) => {
        return row.useCampaigns === false ? "No" : "Si";
    };

    const renderSchedules = (row) => {
        return row.useSchedules === false ? "No" : "Si";
    };

    const renderInternalChat = (row) => {
        return row.useInternalChat === false ? "No" : "Si";
    };

    const renderExternalApi = (row) => {
        return row.useExternalApi === false ? "No" : "Si";
    };
                ...
                <Table size="small">
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell align="center">Tamaño de carpeta</TableCell>
                            <TableCell align="center">Archivos totales</TableCell>
