 ...
 const { user, socket } = useContext(AuthContext);
  ...
  const handleDeleteWebhook = async webhookId => {
    try {
      ...
      toast.success("FlujoBot eliminado con éxito");
    } catch (err) {
      toastError(err);
    }
  };
  ...
  const handleSaveDefault = async () => {
    ...
    if (configExist) {
      try {
        ...
        toast.success("FlujoBots estándar actualizados");
      } catch (err) {
        toastError(err);
      }
    } else {
  ...
  return (
    <MainContainer className={classes.mainContainer}>
      ...
      >
        {deletingContact
          ? `¿Está seguro de que desea eliminar este FlujoBot? Se perderán todas las integraciones relacionadas.`
          : `${i18n.t("contacts.confirmationModal.importMessage")}`}
      </ConfirmationModal>
      <MainHeader>
        <Title>FlujoBot estándar</Title>
      </MainHeader>
      <Paper
      ...
      >
        <Stack sx={{ padding: '12px', position: 'relative' }}>
          ...
          <Stack gap={'12px'}>
            <Typography fontSize={18} fontWeight={700}>Bienvenido FlujoBot</Typography>
            <Typography fontSize={12}>
              Este FlujoBot sólo se activa para nuevos contactos, personas que usted
              no tienes en tu lista de contactos y quien envió un mensaje
            </Typography>
          ...
          <Stack gap={'12px'} marginTop={4}>
            <Typography fontSize={18} fontWeight={700}>FlujoBot de respuesta estándar</Typography>
            <Typography fontSize={12}>
              La respuesta estándar se envía con cualquier carácter que no sea una palabra clave. ¡ATENCIÓN! Se activará si el servicio ya está cerrado y 6 horas después del cierre.
            </Typography>
            {!loading && (<Autocomplete
              disablePortal
              id="combo-box-demo"
