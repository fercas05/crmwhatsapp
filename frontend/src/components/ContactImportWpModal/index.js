...
const ContactImportWpModal = ({ isOpen, handleClose, selectedTags, hideNum, userProfile }) => {
        ...
        setTimeout(async () => {
          try {
            if (index >= contactsToImport?.length - 1) {
              setStatusMessage(`Importación completada exitosamente`)
              //setContactsToImport([])
              setCurrentContact(initialContact)
            ...
            if (index % 5 === 0) {
              ...
              setStatusMessage(`importación en curso ${index} de ${contactsToImport?.length} no salga de esta pantalla hasta que se complete la importación`)
              // toast.info(
              // );
            }
            console.log("antes de importar: ", item[0])
  ...
  return (
    <Dialog fullWidth open={isOpen} onClose={handleClosed}>
      <DialogTitle>{i18n.t("Exportar / Importar contactos")}</DialogTitle>
      <div>
