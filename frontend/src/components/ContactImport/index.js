...
const ContactImport = () => {
  ...
  const processImport = async () => {
    ...
    if (!selection.number) {
      toastError("El campo del número de contacto no fue seleccionado");
      setUploading(false);
      return;
    }

    if (!selection.name) {
      toastError("El campo de nombre de contacto no fue seleccionado");
      setUploading(false);
      return;
    }

    if (Object.keys(selectedRows).length === 0) {
      toastError("No hay contactos seleccionados");
      setUploading(false);
      return;
    }
    ...
    if (rows?.length > 1) {
      for (let index = 1; index < rows.length; index++) {
        if (selectedRows[index]) { // Importar apenas as linhas selecionadas
          ...
          // Verificar se os campos Obligatorios estão presentes
      ...
      if (countIgnored === 0) {
        ...
        toast.alert("La importación fue exitosa, pero hubo algunos errores.");
      }
    }
  };
  ...
  const handleSelectChange = (event) => {
    ...
    if (selectedFields[newValue]) {
      toastError(`el campo ${newValue} ya ha sido seleccionado.`);
      return;
    }
        ...
        <div className={classes.importOptions}>
          <FormGroup row style={{ width: '100%', display: 'flex', justifyContent: 'space-around' }}>
            <FormControlLabel
              ...
              label="Validar contactos en WhatsApp"
            />
          </FormGroup>
        </div>
        {renderXls()}
  ...
  return (
    <div style={{ alignContent: "center" }}>
      {imported && (
        <div>
          <ul>
            <li>{countCreated} contactos creados</li>
            <li>{countIgnored} contactos ignorados (número no válido o no marcado para actualización)</li>
          </ul>
        </div>
      )}
      {openingFile && <div>Archivo de procesamiento...</div>}
      {invalidFile && <div>¡Archivo no válido!</div>}
      {!imported && rows && columns ? renderContent() : (
        <>
          ...
          >
            ...
            <h5>Haga clic o arrastre un archivo</h5>
            <p style={{ color: "#e74c3c", fontWeight: "bold", textAlign: "center" }}>
              * Importante: Sólo se aceptan archivos con extensiones: xls, xslx, csv, txt
            </p>
          </div>
