...
const FlowBuilderSingleBlockModal = ({
  ...
  const [labels, setLabels] = useState({
    title: "Agregar contenido al flujobot",
    btn: "Agregar",
  });
  ...
  const handleElements = (newNameFiles) => {
    ...
    for (let i = 0; i < numberInterval; i++) {
      ...
      if (parseInt(value) === 0 || parseInt(value) > 120) {
        toast.error("¡El rango no puede ser 0 o mayor que 120!");
        setLoading(false);
        throw "";
      }
      elementsSequence.push({
  ...
  const handleChangeMediasImg = (e, number) => {
    ...
    if (e.target.files[0].size > 2000000) {
      toast.error("¡El archivo es demasiado grande! 2 MB máximo");
      return;
    }
  ...
  const handleChangeAudios = (e, number) => {
    ...
    if (e.target.files[0].size > 5000000) {
      toast.error("¡El archivo es demasiado grande! 5 MB máximo");
      return;
    }
  ...
  const handleChangeVideos = (e, number) => {
    ...
    if (e.target.files[0].size > 20000000) {
      toast.error("¡El archivo es demasiado grande! 20 MB máximo");
      return;
    }
      ...
      >
        ...
        <Stack direction={"row"} justifyContent={"center"}>
          ...
          <Stack justifyContent={"center"}>
            <Typography>Enviar como audio grabado instantáneamente</Typography>
          </Stack>
        </Stack>
      </Stack>
    );
  ...
  const handleSaveNode = async () => {
    if (open === "edit") {
      ...
      setTimeout(async () => {
        ...
        if (verify) {
          setLoading(false);
          return toast.error("Eliminar tarjetas vacías (Imagen, Audio y Video)");
        }
        await api
      ...
      <Dialog open={activeModal} fullWidth="md" scroll="paper">
        ...
        {loading && (
          ...
          >
            <Stack>
              <Typography>
                Subir archivos y crear contenido...
              </Typography>
