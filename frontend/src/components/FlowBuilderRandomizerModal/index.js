...
const FlowBuilderRandomizerModal = ({
  ...
  const handleSaveContact = async values => {
    if (!percent || parseInt(percent) <= 0) {
      return toast.error("Agregar valor de rango");
    }
    if (parseInt(percent) > 120) {
      return toast.error("El tiempo máximo alcanzó los 120 segundos.");
    }
    if (open === "edit") {
      onUpdate({
        ...data,
    ...
    <div className={classes.root}>
      ...
      >
        <DialogTitle id="form-dialog-title">
          {open === "create"
            ? `Añade un aleatorizador a tu flujo`
