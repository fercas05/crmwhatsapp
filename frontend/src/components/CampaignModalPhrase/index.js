...
const CampaignModalPhrase = ({ open, onClose, FlowCampaignId, onSave }) => {
  ...
  const applicationSaveAndEdit = () => {
    ...
    if (FlowCampaignId) {
      ...
      onClose();
      onSave('ok');
      toast.success("¡La oración cambió exitosamente!");
      clearData();
    } else {
      ...
      >
        <DialogTitle id="form-dialog-title">
          {campaignEditable ? (
            <>
              {FlowCampaignId
                ? `Editar campaña con flujo de frases`
                : `Nueva campaña con flujo de frases.`}
            </>
          ) : (
        ...
        {!loading && (
          <Stack sx={{ padding: "52px" }}>
            <Stack sx={{ gap: "14px" }}>
              ...
              <Stack gap={1}>
                <Typography>Elige una transmisión</Typography>
                <Autocomplete
                  disablePortal
                  id="combo-box-demo"
                  value={flowSelected}
              ...
              <Stack gap={1}>
                <Typography>¿Qué frase desencadena el flujo?</Typography>
                <TextField
                  label={""}
                  name="text"
                  variant="outlined"
