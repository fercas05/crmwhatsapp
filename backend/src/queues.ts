...
async function handleSendScheduledMessage(job) {
  ...
  try {
    ...
    Sentry.captureException(e);
    logger.info(`Error al intentar consultar el recordatorio: ${schedule.id}`);
  }
    ...
    if (campaigns.length > 0) {
      ...
      const promises = campaigns.map(async (campaign) => {
        try {
          ...
          logger.info(
            `Campaña enviada al departamento de procesamiento: Campanha=${campaign.id},  Delay Initial=${delay}`
          );
      ...
      logger.info('Todas las campañas han sido procesadas y agregadas al departamento.');
    }
  } catch (err) {
    Sentry.captureException(err);
...
const checkTime = async () => {
  ...
  logger.info(
    `Comienza el envío ${hour} y termina el ${endHours}, hora actual ${timeNow} no en horario`
  );
    ...
    if (campaign) {
      ...
      if (isArray(contacts)) {
        ...
        for (let i = 0; i < contactData.length; i++) {
          ...
          logger.info(`Inscripción enviada para línea de envio.: Campanha=${campaign.id};Contato=${contacts[i].name};delay=${delay}`);
          // }
        }
...
async function handleDispatchCampaign(job) {
  try {
    ...
    logger.info(
      `Envio de campaña solicitado: Campanha=${campaignId};Registro=${campaignShippingId}`
    );
...
export async function startQueueProcess() {
  logger.info("Iniciando procesamiento del departamento");
