...
export const remove = async (
  ...
  try {
    ...
    if (count > 0) return res.status(200).json({ message: "¡No se puede eliminar! ¡Compruebe si este Prompt se utiliza en las conexiones de Whatsapp!" });
  ...
  .emit(`company-${companyId}-prompt`, {
    ...
    return res.status(500).json({ message: "¡No se puede eliminar! ¡Compruebe si se está utilizando este Prompt!" });
  }
};
