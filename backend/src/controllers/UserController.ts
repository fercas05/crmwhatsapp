  ...
  if (!companyUser) {
    ...
    try {
      const _email = {
        ...
        Siga los datos de su empresa:<br><br>Nombre: ${companyName}<br>Email: ${email}<br>Contraseña: ${password}<br>Fecha de vencimiento de la prueba: ${dateToClient(date)}`
      }

      await SendMail(_email)
    } catch (error) {
      console.log('no pude enviar el correo')
    }
