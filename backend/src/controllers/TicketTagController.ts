...
export const remove = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Las etiquetas de los chat se eliminaron correctamente.' });
  } catch (error) {
    return res.status(500).json({ error: 'No se pudieron eliminar las etiquetas de los chat.' });
  }
};